...
var hist_NDWINoMask = ui.Chart.image.histogram({image:NWINoMask.first(), region: geometry, scale: 11})
  .setOptions({title: 'Histograma primeiro MNDWI sem máscara'});
...
var hist_NDWI_Mask = ui.Chart.image.histogram({image:NWI.first(), region: geometry, scale: 11})
  .setOptions({title: 'Histograma primeiro MNDWI com máscara'});
...
var task = Export.table.toDrive({
  collection: vectors,
  description:'Intertidal_Zones_joinville_MNDWI',
...
Export.image.toDrive({
  ...
  folder: 'Intertidal_Zones', 
  fileNamePrefix:'img_ndwi_joinville_MNDWI',
  region:geometry, 
  scale: 10,
  maxPixels: 100e9});
